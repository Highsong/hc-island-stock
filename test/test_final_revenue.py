    ...
    try:
        ...
        if income_data:
            ...
            if '2025' in income_data:
                ...
                revenue = data_2025['营业收入'][0]
                net_profit = data_2025['净利润'][0]

                print(f"2025年营业收入: {revenue:,.2f} ({revenue/1e8:.2f}亿)")
                print(f"2025年净利润: {net_profit:,.2f} ({net_profit/1e8:.2f}亿)")
                ...
                if abs(revenue - expected_revenue) < tolerance:
                    print(f"[SUCCESS] 营业收入匹配预期值1688.38亿！")
                else:
                    print(f"[FAIL] 营业收入不匹配预期值1688.38亿")
                    print(f"  差值: {abs(revenue - expected_revenue):,.2f}")
